...
public class UserServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (btn != null) {
                String emails = request.getParameter("option");
                String status = request.getParameter("status");
                double paid = Double.parseDouble(request.getParameter("paid"));
                double unpaid = Double.parseDouble(request.getParameter("unpaid"));
                java.sql.Timestamp dateTime = new java.sql.Timestamp(new java.util.Date().getTime());

                Service updatePay = new PaymentService();

                updatePay.updatePayment(status, paid, unpaid, dateTime, dateTime, emails);
